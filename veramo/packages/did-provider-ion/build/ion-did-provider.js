...
import { IonDidForm, KeyIdentifierRelation, KeyType, } from './types/ion-provider-types.js';
import { IonSigner } from './ion-signer.js';
import { resolveDidIonFromIdentifier } from './ion-did-resolver.js';
import { IonPublicKeyPurpose, IonRequest } from '@decentralized-identity/ion-sdk';
import { computeCommitmentFromIonPublicKey, generatePrivateKeyHex, getActionTimestamp, getVeramoRecoveryKey, getVeramoUpdateKey, ionDidSuffixFromLong, ionLongFormDidFromCreation, ionShortFormDidFromLong, tempMemoryKey, toIonPublicKey, toIonPublicKeyJwk, toJwkEs256k, truncateKidIfNeeded, } from './functions.js';
...
export class IonDIDProvider extends AbstractIdentifierProvider {
    ...
    async createIdentifier({ kms, options, alias }, context) {
        ...
        const veramoKeys = [recoveryKey, updateKey];
        const ionPublicKeys = [];
    ...
    async removeKey({ identifier, kid, options }, context) {
        const rotation = await this.rotateVeramoKey({ identifier, options, context });
        const request = await IonRequest.createUpdateRequest({
            didSuffix: ionDidSuffixFromLong(identifier.did),
            updatePublicKey: rotation.currentJwk,
    ...
    async removeService({ identifier, id, options }, context) {
        const rotation = await this.rotateVeramoKey({ identifier, options, context });
        const request = await IonRequest.createUpdateRequest({
            didSuffix: ionDidSuffixFromLong(identifier.did),
            updatePublicKey: rotation.currentJwk,
     ...
     */
    async getAssertedDidDocument(identifier, didForm = IonDidForm.LONG) {
     ...
     */
    async rotateUpdateOrRecoveryKey({ identifier, commitment, relation, kms, options, actionTimestamp, }, context) {
        const currentVeramoKey = relation == KeyIdentifierRelation.UPDATE
     ...
     * @param identifier - The identifier (DID) for which to update the recovery/update key
     */
    async rotateVeramoKey({ kms, context, options, identifier, }) {
        const didResolution = await this.getAssertedDidDocument(identifier, IonDidForm.LONG);
     ...
     * @param context - The Veramo agent context
     */
    async importProvidedOrGeneratedKey({ kms, actionTimestamp, relation, options, }, context) {
     ...
     */
    async anchorRequest(request, anchor) {
        if (anchor !== false) {
     ...
     */
    async deleteKeyOnError(kid, context) {
        try {
...
//# sourceMappingURL=ion-did-provider.js.map