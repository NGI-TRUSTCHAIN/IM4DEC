...
const execute = new Command('execute')
    ...
    try {
            ...
            if (methodApi.arguments.type === 'object') {
                for (const property in methodApi.arguments.properties) {
                    ...
                    switch (propertySchema.type) {
                        case 'object':
                            question.type = 'input';
                            question.filter = (input) => JSON.parse(input === '' ? '{}' : input);
                            question.transformer = (input) => JSON.stringify(input);
                            break;
                        case 'string':
                            question.type = 'input';
                            break;
        ...
        console.log('\nResult', methodApi.returnType.description ? `(${methodApi.returnType.description}):` : ':', JSON.stringify(result, null, 2));
    }
    catch (e) {
