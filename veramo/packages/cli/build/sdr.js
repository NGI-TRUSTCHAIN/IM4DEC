    ...
    if (answers5.addCredentials) {
        ...
        if (vcs.length > 0) {
            for (const credential of vcs) {
                list.push({
                    name: JSON.stringify(credential.verifiableCredential.credentialSubject) +
                        ' | Issuer: ' +
...
sdr
    ...
    const list = sdrMessages.map((message) => ({
        //FIXME
        name: shortDate(message.createdAt) +
            ' ' +
    ...
    for (const item of credentialsForSdr) {
        questions.push({
            ...
            choices: item.credentials.map((c) => ({
                name: c.verifiableCredential.credentialSubject[item.claimType] +
                    ' (' +
